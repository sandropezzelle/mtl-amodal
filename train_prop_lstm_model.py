...
if __name__ == '__main__':
    ...
    weights = "best_prop_lstm_model/best-weight-model.hdf5"
    ...
    model = prop_lstm_model.PropLSTMModel(embeddings, token2id).build()
    ...
    hist = model.fit(
        ...
        epochs=num_epochs,
        validation_data=(dataset_v, v_r_out),
        callbacks=[checkpoint]
    )

    best_model = prop_lstm_model.PropLSTMModel(embeddings, token2id).build()
    best_model.load_weights(weights)
    print(best_model.evaluate(dataset_t, t_r_out, batch_size=batch_size))
